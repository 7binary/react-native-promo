...
import CardsWidget from 'modules/prizes/components/CardsWidget';
import styles from './styles';
import { profileIdSelector } from 'modules/auth/store';

interface ICatalogSetting {
...
const CatalogSetting: React.FC<ICatalogSetting> = ({ setType, logoSource, isActive }) => {
  const borderColor = isActive ? Settings.colors.primary : '#CCC';
  const color = isActive ? Settings.colors.primary : '#666';
  ...
  return (
    ...
    >
      ...
      <Text style={{ ...styles.selectTitle, color }}>Сертификаты</Text>
    </TouchableOpacity>
  );
};
...
const PrizesWidget = () => {
  ...
  const [type, setType] = useState<PaymentType>('catalog');
  ...
  return (
    <View>
      <View style={styles.selectContainer}>
        <CatalogSetting
          isActive={type === 'catalog'}
          logoSource={require('assets/images/payment-catalog.png')}
